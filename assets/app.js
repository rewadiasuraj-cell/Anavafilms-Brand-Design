...
function work(){return `<div class="work-page"><section class="work-masthead"><div class="work-heading-new"><div><span class="eyebrow">OUR WORK</span><h1>Curated Work.<br><em>Real Impact.</em></h1></div><p>A selection of films, content and campaigns we’ve created for brands across industries.</p></div><div class="archive-categories" aria-label="Project categories">${workCategories.map((c,i)=>`<button data-archive="${c}" aria-pressed="${i===0}" class="${i===0?'selected':''}">${categoryName(c)}</button>`).join('')}</div></section><section class="work-archive" id="work-archive"><div id="archive-results">${archiveCollection()}</div></section><section class="work-end" aria-label="Start your next project"><div class="work-end-media" aria-hidden="true"><img src="assets/what brand say.jpg" alt="" loading="lazy" width="1600" height="1000"></div><div class="work-end-content"><span class="work-end-eyebrow">YOUR NEXT PROJECT<span class="work-end-eyebrow-line" aria-hidden="true"></span></span><h2><span class="next-title-line"><span>Let’s make</span></span><span class="next-title-line next-title-accent"><span>The Next one <span class="next-title-arrow" aria-hidden="true">↗</span></span></span></h2><p class="work-end-copy">Got a project in mind?<br>Let’s create something great together.</p><a class="work-end-cta" href="#contact"><span>Let’s Talk</span><span class="work-end-cta-arrow" aria-hidden="true">→</span></a></div></section></div>`}
...
function animate(){if(motion)motion.revert();if(!window.gsap||!window.ScrollTrigger)return;gsap.registerPlugin(ScrollTrigger);motion=gsap.matchMedia();motion.add({allowed:'(prefers-reduced-motion: no-preference)',desktop:'(min-width: 761px) and (hover: hover) and (pointer: fine)'},ctx=>{if(!ctx.conditions.allowed)return;if(document.querySelector('.hero-line'))gsap.from('.hero-line',{y:35,opacity:0,duration:MOTION.reveal,stagger:MOTION.stagger,ease:MOTION.ease});gsap.utils.toArray('.reveal').forEach(el=>gsap.from(el,{y:ctx.conditions.desktop?24:12,opacity:0,duration:ctx.conditions.desktop?MOTION.reveal:.4,ease:MOTION.ease,scrollTrigger:{trigger:el,start:'top 94%',once:true}}));if(document.querySelector('.work-heading h1'))gsap.from('.work-heading h1',{y:30,opacity:0,duration:MOTION.reveal,ease:MOTION.ease});if(document.querySelector('.motion-line'))gsap.to('.motion-line div',{xPercent:-25,ease:'none',scrollTrigger:{trigger:'.motion-line',start:'top bottom',end:'bottom top',scrub:MOTION.scrub}});if(!ctx.conditions.desktop)return;gsap.utils.toArray('.card-image img').forEach(el=>{gsap.fromTo(el,{yPercent:-8},{yPercent:0,ease:'none',scrollTrigger:{trigger:el.parentElement,start:'top bottom',end:'bottom top',scrub:MOTION.scrub}});gsap.fromTo(el,{opacity:.4},{opacity:1,duration:1.1,ease:'sine.inOut',scrollTrigger:{trigger:el.parentElement,start:'top 92%',once:true}})});if(document.querySelector('.film')){gsap.fromTo('.film img,.film video',{yPercent:-10},{yPercent:0,ease:'none',scrollTrigger:{trigger:'.film-stage',start:'top bottom',end:'bottom top',scrub:MOTION.scrub}});gsap.fromTo('.film img,.film video',{opacity:.35},{opacity:1,duration:1.3,ease:'sine.inOut'})}if(document.querySelector('.work-end')){const lines=gsap.utils.toArray('.work-end .next-title-line>span');gsap.timeline({scrollTrigger:{trigger:'.work-end',start:'top 75%',once:true}}).from('.work-end-media img',{scale:1.04,opacity:.75,duration:1.4,ease:'power3.out'}).from('.work-end-eyebrow',{opacity:0,y:16,duration:.6,ease:MOTION.ease},.05).from('.work-end-eyebrow-line',{scaleX:0,transformOrigin:'left center',duration:.7,ease:MOTION.ease},.1).from(lines,{yPercent:110,duration:ctx.conditions.desktop?1:.7,stagger:.13,ease:'power4.out'},.15).from('.work-end-copy',{opacity:0,y:18,duration:.7,ease:MOTION.ease},.5).from('.work-end-cta',{opacity:0,y:20,scale:.98,duration:.65,ease:'power3.out'},.6);if(ctx.conditions.desktop)gsap.fromTo('.work-end-media img',{yPercent:-4},{yPercent:4,ease:'none',scrollTrigger:{trigger:'.work-end',start:'top bottom',end:'bottom top',scrub:1}})}if(document.querySelector('.contact-hero')){const lines=gsap.utils.toArray('.contact-hero .thought-line-mask>span');gsap.timeline({scrollTrigger:{trigger:'.contact-hero',start:'top 78%',once:true}}).from('.contact-hero-media img',{scale:1.035,opacity:.82,duration:1.4,ease:'power3.out'}).from('.contact-eyebrow',{opacity:0,y:14,duration:.6,ease:MOTION.ease},.05).from('.contact-eyebrow-line',{scaleX:0,transformOrigin:'left center',duration:.65,ease:MOTION.ease},.1).from(lines,{yPercent:115,duration:ctx.conditions.desktop?.95:.7,stagger:.12,ease:'power4.out'},.15).from('.contact-hero-copy',{opacity:0,y:16,duration:.7,ease:MOTION.ease},.5).from('.contact-hero-cta',{opacity:0,y:18,scale:.98,duration:.65,ease:'power3.out'},.6);if(ctx.conditions.desktop)gsap.fromTo('.contact-hero-media img',{yPercent:-1},{yPercent:1,ease:'none',scrollTrigger:{trigger:'.contact-hero',start:'top bottom',end:'bottom top',scrub:1}})}})}

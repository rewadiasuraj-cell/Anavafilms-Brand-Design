...
const clientLogosList=[
  {name:'Godrej Properties',logo:'assets/clients/godrej-properties.png'},
  {name:'Lenskart',logo:'assets/clients/lenskart.png'},
  {name:'KFC',logo:'assets/clients/kfc.png'},
  {name:'Tira Beauty',logo:'assets/clients/tira.png'},
  {name:'Celio',logo:'assets/clients/celio.png'},
  {name:'WOW Skin Science',logo:'assets/clients/wow-skin-science.png'},
  {name:'FIKN Elixir',logo:'assets/clients/fikn.png'},
  {name:'SimplAI',logo:'assets/clients/simplai.png'},
  {name:'Indus Valley',logo:'assets/clients/indus-valley.png'},
  {name:'Green Lotus',logo:'assets/clients/green-lotus.png'}
];
const clientLogoItem=c=>`<div class="logo-item"><img src="${c.logo}" alt="${c.name}" loading="lazy" height="32"><span class="logo-divider" aria-hidden="true"></span></div>`;
...
function home(){return `<section class="hero"><span class="eyebrow reveal">Independent thinking. Collective making.</span><h1><span class="hero-line">Give us a thought.</span><br><span class="hero-line">We’ll give you</span><br><em class="hero-line">ideas to shoot.</em></h1><p class="reveal">Anava Films takes a thought all the way to the final frame — creative direction, scripting, production and post, all in one place.</p><a class="pill" href="#work">Explore our work ${arrow}</a><div class="hero-bottom"><span>MUMBAI & DELHI · AVAILABLE EVERYWHERE</span><span>SCROLL TO EXPLORE ↓</span></div></section><section class="film-stage"><div class="film"><img src="assets/tvc/kareena-kapoor-tira-beauty.jpg" alt="Kareena Kapoor x Tira Beauty — creative direction" fetchpriority="high" width="1600" height="1100"><div class="film-label"><p>THE ART OF MAKING YOU LOOK.</p><h2>A little unexpected.<br>Entirely unforgettable.</h2></div><a class="play" href="#work" aria-label="Explore our work">${arrow}</a></div></section>${clientLogos()}<section class="section intro"><div><span class="eyebrow">OUR PHILOSOPHY</span><div class="intro-photo reveal"><img src="assets/thinkers-who-make.jpg" alt="Thinkers who make" loading="lazy" width="576" height="720"></div></div><div><h2 class="reveal">Thinkers who make.<br>Makers who think.</h2><p class="reveal">Anava Films sits somewhere between an agency and a production house. We believe great ideas shouldn’t get lost between the people who think them and the people who make them.</p><p class="reveal">So we bring both sides together. You bring the thought. We build the idea, make the film and deliver the final product.</p><div class="philosophy-process eyebrow reveal">THINK → MAKE → DELIVER</div><a class="pill reveal" href="#work">WATCH THE SHOWREEL 2026 ${arrow}</a></div></section><section class="section"><div class="section-heading"><div><span class="eyebrow">02 / Selected perspectives</span><h2 class="reveal">Work that stays<br>with you.</h2></div><a class="text-link" href="#work">All work ↗</a></div><div class="grid">${['sunil-shetty-brand-film','lenskart-hustler'].map(id=>A.projects.find(p=>p.id===id)).filter(Boolean).map(card).join('')}</div></section>${serviceSection()}<div class="motion-line" aria-hidden="true"><div>Thought. <em>Idea.</em> Action. Thought. <em>Idea.</em> Action.</div></div><section class="section"><div class="section-heading"><div><span class="eyebrow">03 / The way we work</span><h2 class="reveal">From thought<br>to screen.</h2></div><a class="text-link" href="#process">Our process ↗</a></div><div class="process-list">${[steps[0],steps[1],steps[4]].map((s,i)=>`<article class="process-item reveal"><small>0${i+1}</small><h3>${s[0]}</h3><p>${s[1]}</p></article>`).join('')}</div></section><section class="section"><span class="eyebrow">04 / The people behind the frame</span><h2 class="big reveal">Good people.<br>Better together.</h2>${people()}</section>${locationSection()}`}
...
const archiveMedia=p=>p.video?`<video class="lazy-video archive-video" muted loop playsinline preload="none" poster="${p.image||''}" data-src="${p.video}" aria-label="${p.title}"></video>`:p.raw&&supportsHoverReveal()?`<img class="archive-raw" src="${p.raw}" alt="${p.title}" loading="lazy" width="1200" height="900"><img class="archive-edited" ${responsiveImage(p.image)} src="${p.image}" alt="" aria-hidden="true" loading="lazy" width="1200" height="900">`:p.image?`<img ${responsiveImage(p.image)} src="${p.image}" alt="${p.title}" loading="lazy" width="1200" height="900">`:'';
function archiveCard(p,i){const hasVideo=!!p.video,mode=videoMode(p.category);return `<button class="archive-card ${isPortrait(p)?'portrait':''}" data-project="${p.id}"${hasVideo?` data-video-mode="${mode}"`:''}><div class="archive-image">${archiveMedia(p)}<span class="archive-badge">${(p.client||'').toUpperCase()}</span>${hasVideo?`<span class="archive-mute" role="button" tabindex="0" aria-label="Unmute video" aria-pressed="false">${mutedIcon}</span>`:''}${hasVideo?`<span class="archive-play" aria-hidden="true"><svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg></span>`:''}</div><div class="archive-caption-new"><h3>${p.title.toUpperCase()}</h3><p class="archive-format">${p.format||categoryName(p.category)}</p>${p.tagline?`<p class="archive-tagline">${p.tagline}</p>`:''}</div></button>`}
...
function servicePage(){return `<section class="services-hero"><span class="eyebrow reveal">WHAT WE DO</span><h1 class="reveal">From Idea to <em class="accent-text">Impact.</em></h1><p class="reveal">End-to-end creative solutions that turn ideas into powerful visual stories.</p></section><section class="film-stage service-hero-stage"><div class="film"><img src="assets/think.jpg" alt="Behind the scenes on set" loading="lazy" width="1600" height="1000"><div class="film-label"><p class="eyebrow">WHAT WE DO</p><h2>From Idea to<br><em class="accent-text">Impact.</em></h2></div></div></section><section class="section service-cards-section"><div class="service-cards-grid"><article class="service-card reveal"><div class="service-card-head"><div class="service-card-icon" aria-hidden="true">${bulbIcon}</div><div class="service-card-tags"><span>IDEATION</span><span>CONCEPT</span><span>SCRIPTING</span></div></div><div class="service-card-body"><span class="service-step-label">01 / CONCEPT</span><h3 class="service-card-title">THINK</h3><p class="service-card-desc">Creative Direction, Ideation &amp; Concept Development, Scripting, Creative Consulting.</p></div><div class="service-card-foot"><a href="#process/step-1" class="service-explore-link">EXPLORE THINK →</a></div></article><article class="service-card reveal"><div class="service-card-head"><div class="service-card-icon" aria-hidden="true">${clapperIcon}</div><div class="service-card-tags"><span>PRODUCTION</span><span>DIRECTION</span><span>CASTING</span></div></div><div class="service-card-body"><span class="service-step-label">02 / SHOOT</span><h3 class="service-card-title">MAKE</h3><p class="service-card-desc">Production, Direction, Casting (Domestic &amp; International), Art Direction &amp; Production Design.</p></div><div class="service-card-foot"><a href="#process/step-5" class="service-explore-link">EXPLORE MAKE →</a></div></article><article class="service-card reveal"><div class="service-card-head"><div class="service-card-icon" aria-hidden="true">${finishIcon}</div><div class="service-card-tags"><span>EDITING</span><span>VFX</span><span>COLOR GRADING</span></div></div><div class="service-card-body"><span class="service-step-label">03 / POST</span><h3 class="service-card-title">FINISH</h3><p class="service-card-desc">Editing, VFX, Sound Design, Color Grading, Motion Graphics &amp; Final Delivery.</p></div><div class="service-card-foot"><a href="#process/step-6" class="service-explore-link">EXPLORE FINISH →</a></div></article></div></section>${servicesCta()}<div class="motion-line" aria-hidden="true"><div>THINK. <em>MAKE.</em> FINISH. THINK. <em>MAKE.</em> FINISH. THINK. <em>MAKE.</em> FINISH. THINK. <em>MAKE.</em> FINISH.</div></div>`}
...
const locations=[['Mumbai',true,'301, Creative House,<br>Linking Road, Bandra West,<br>Mumbai - 400050','https://maps.google.com/?q=Linking+Road+Bandra+West+Mumbai+400050','Same City<br>Bigger Stories','loc-mumbai','assets/locations/mumbai-gateway.jpg'],['Delhi',false,'Studio 8, Creative Block,<br>Saket, New Delhi - 110017','https://maps.google.com/?q=Saket+New+Delhi+110017','More Ideas<br>More Films','loc-delhi','assets/locations/delhi-india-gate.jpg']];
function locationSection(){return `<section class="locations-section" aria-label="Our studios"><span class="eyebrow">Where we work</span><h2 class="reveal">Two cities.<br>One studio.</h2><div class="locations-grid">${locations.map(l=>`<div class="location-tile ${l[5]} reveal"><div class="location-tile-bg"><img src="${l[6]}" alt="${l[0]} landmark" loading="lazy" width="800" height="450"></div><div class="location-tile-head"><h3>${l[0]}</h3>${l[1]?'<span class="hq-tag">HQ</span>':''}</div><span class="location-tag">${l[4]}</span><p class="location-address">${l[2]}</p><a class="connect-link" href="${l[3]}" target="_blank" rel="noopener"><span>Get Directions ${arrow.replace('width="20" height="20"','width="15" height="15"')}</span></a></div>`).join('')}</div></section>`}
